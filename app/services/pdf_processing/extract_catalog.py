...
# ── Spatial image matching tuneable constants ───────────────────────────────
MIN_IMAGE_WIDTH   = 60    # px — skip tiny icons / bullets
MIN_IMAGE_HEIGHT  = 60
MAX_PAGE_FRACTION = 0.85  # skip full-page background images
PROXIMITY_RADIUS  = 250   # pt — how close text must be to image centre
...
class CatalogExtractor:
    ...
    def process_pdf(self, pdf_path: str, output_image_dir: str) -> List[Dict[str, Any]]:
        ...
        try:
                    ...
                    try:
                        ...
                        for part in parts:
                            part_data = {
                                ...
                                'image_path': None  # Will be filled by spatial image matching
                            }
            ...
            # Extract images using spatial proximity matching (improved)
            self._extract_part_images_spatial(pdf_path, output_image_dir, pdf_name, catalog_data)
    ...
    # ------------------------------------------------------------------
    # IMPROVED: Spatial image-to-part matching
    # Replaces the old _extract_part_images which used broken zero coords
    # ------------------------------------------------------------------

    def _extract_part_images_spatial(
        self,
        pdf_path: str,
        ...
        """
        For every embedded image in the PDF:
          1. Find the part-number text that is spatially closest to the image.
          2. Save the image file.
          3. Set image_path on every catalog_data entry that shares that
             part_number AND page (true 1-to-1 linking).

        Modifies catalog_data in-place.
        """
        try:
            ...
            # Build fast lookup: (page, part_number_upper) → list of entries
            entry_index: Dict[tuple, List[Dict]] = {}
            for entry in catalog_data:
                key = (entry.get("page"), entry.get("part_number", "").upper())
                entry_index.setdefault(key, []).append(entry)
            ...
            for page_num in range(len(doc)):
                ...
                # Collect all text spans with bounding-box centres
                text_spans = []
                ...
                for img_index, img in enumerate(page.get_images(full=True)):
                    try:
                        ...
                        # Skip CMYK, too-small, and full-page images
                        if pix.n - pix.alpha >= 4:
                            pix = None; continue
                        if pix.width < MIN_IMAGE_WIDTH or pix.height < MIN_IMAGE_HEIGHT:
                            pix = None; continue
                        if (pix.width * pix.height) > (page_w * page_h * MAX_PAGE_FRACTION):
                            pix = None; continue

                        # Get real placement rectangle on the page
                        ...
                        # Find nearest text span within PROXIMITY_RADIUS
                        best_text = None
                        best_dist = float("inf")
                        for span in text_spans:
                            dist = ((span["cx"] - icx) ** 2 + (span["cy"] - icy) ** 2) ** 0.5
                            if dist < best_dist and dist <= PROXIMITY_RADIUS:
                                best_dist = dist
                                best_text = span["text"]
                        ...
                        rel_path   = str(image_path.relative_to(Path(output_image_dir).parent))
                        total_saved += 1

                        # Associate image with the closest matching part entry
                        matched = False
                        if best_text:
                            clean = best_text.upper().strip()
                            for (pg, pnum), entries in entry_index.items():
                                if pg == page_num + 1 and (
                                    pnum == clean or clean in pnum or pnum in clean
                                ):
                                    for e in entries:
                                        if not e.get("image_path"):
                                            e["image_path"] = rel_path
                                    matched = True

                        # Fallback: assign to any part on this page still missing an image
                        if not matched:
                            for (pg, _), entries in entry_index.items():
                                if pg == page_num + 1:
                                    for e in entries:
                                        if not e.get("image_path"):
                                            e["image_path"] = rel_path
                        ...
                        logger.debug(
                            ...
                            f"({'matched: ' + best_text if matched and best_text else 'page-fallback'})"
                        )
                        pix = None
            ...
            doc.close()
            logger.info(f"Spatial image extraction: {total_saved} images saved for {pdf_name}")

        except Exception as e:
            logger.error(f"Error in _extract_part_images_spatial for {pdf_path}: {e}")

    # ------------------------------------------------------------------
    # Keep old method as an alias so nothing else breaks if referenced
    # ------------------------------------------------------------------
    def _extract_part_images(self, pdf_path, output_image_dir, pdf_name, catalog_data):
        """Deprecated — delegates to the improved spatial version."""
        self._extract_part_images_spatial(pdf_path, output_image_dir, pdf_name, catalog_data)

    def _is_part_image(self, img_info: tuple, page: fitz.Page, page_text: str, label_regexes: list) -> bool:
        """Legacy helper — kept for backwards compatibility but no longer called internally."""
        try:
            img_rect = img_info[1:5]
            if len(img_rect) >= 4:
    ...
    def extract_table_of_contents(self, pdf_path: str) -> list:
    ...
    def process_and_upload_to_s3(
        self, pdf_path: str, output_image_dir: str, upload_to_s3: bool = False
    ) -> List[Dict[str, Any]]:
        ...
        if upload_to_s3:
            try:
                ...
                data_filename = (
                    f"{Path(pdf_path).stem}_processed_"
